...
public class GameUI : MonoBehaviour
{
    ...
    void DrawTooltip(float x, float y)
    {
        ...
        if (fromInv) {
            bool isConsumable = n.Contains("Bình Máu");
            string btnText = isConsumable ? "🧪 SỬ DỤNG" : "🛡 MẶC / DÙNG";
            GUI.color = isConsumable ? Color.yellow : Color.green;
            
            if (GUI.Button(new Rect(x+10, y+240, 200, 30), btnText)) { 
                if (isConsumable) currentView.UseConsumable(selectedItemIdx);
                else currentView.EquipItem(selectedItemIdx); 
                ResetSelection(); 
            }
    ...
    void DrawHUD() {
        ...
        // Hiện số lượng bình máu phím tắt
        if (player != null)
        {
            int potionCount = 0;
            foreach (var item in player.inventory) if (item.Contains("Bình Máu")) potionCount++;
            GUI.color = Color.white;
            GUI.Label(new Rect(280, 15, 200, 20), "🧪 [H] HỒI MÁU: " + potionCount, new GUIStyle(GUI.skin.label){fontStyle=FontStyle.Bold, fontSize=12});
        }
