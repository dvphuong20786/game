...
- [x] **1. Tab Animator (Quan trọng nhất):** Phải có 2 Parameter là **`Speed`** (Float) và **`Attack`** (Trigger). Nếu thiếu, Hiệp sĩ sẽ đứng im như tượng đá.
- [x] **2. Script Lò Đẻ (MonsterSpawner):** Chọn lò đẻ trên Hierarchy -> Nhìn bảng Inspector -> Kéo con Quái mẫu (Prefab) vào ô **Monster Prefab**.
- [x] **3. Script Rơi Đồ (Monster):** Chọn con Quái (Prefab) -> Inspector -> Tìm ô **Item Drop Prefab** -> Kéo cục Vàng mẫu vào đó để khi chết nó có cái mà rớt ra.
- [x] **4. Script Bám Đuôi (Main Camera):** Như đã nói ở trên, phải kéo script **`CameraFollow`** vào Camera.
...
- [x] **Bước 2:** Chỉnh **Sorting Layer** thành cùng layer with Player để không bị che khuất.
...
## 11. HƯỚNG DẪN CÀI ĐẶT LỖ QUÁI VẬT (MONSTER PIT)

Đây là cơ chế đẻ quái nâng cao, giúp bản đồ trông sống động và có quy luật hơn:

- **Bước 1:** Tạo một GameObject trống (Create Empty) đặt tên là `MonsterPit_Slime`.
- **Bước 2:** Kéo script `MonsterPit.cs` thả vào nó.
- **Bước 3:** Chỉnh các thông số trong Inspector:
    - **Monster Prefab:** Kéo Slime Prefab vào.
    - **Max Monsters:** Đặt là 4 hoặc 5 (tùy bạn).
    - **Thoi Gian Hoi Sinh:** Thời gian để 1 con quái mới sinh ra sau khi có quái chết.
    - **Aggro Range:** Mặc định 10-12. Đây là tầm quái "nhìn thấy" bạn.
    - **Reset Range:** Mặc định 20-24. Khi bạn chạy quá tầm này, quái sẽ bỏ cuộc và quay về lỗ.
- **Bước 4:** Gán Sprite `monster_hole` (trong thư mục AI_Generated_Sprites) vào đối tượng này để nó hiện ra cái lỗ trên mặt đất.

> [!TIP]
> **Tấn công Đệ tử:** Bạn không cần làm gì thêm, quái vật giờ đây đã thông minh hơn, nếu đệ tử đứng gần nó sẽ "cắn" đệ tử thay vì chạy đi tìm bạn ở xa.

## 12. MẸO QUẢN LÝ TRANG BỊ NHANH

Trong bảng Túi đồ (phím B), khi bạn nhấp vào một món trang bị của Người chơi, bạn sẽ thấy nút:
- **TRANG BỊ CHO ĐỆ:** Một nút tắt ma thuật giúp mặc ngay món đồ đó cho đệ tử của bạn mà không cần chuyển túi đồ qua lại.